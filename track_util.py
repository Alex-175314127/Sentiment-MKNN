...
def create_emotSen_table():
	c.execute('CREATE TABLE IF NOT EXISTS HistoryTable(rawtext TEXT,Sentiment TEXT,sentiment_score NUMBER,Emotion TEXT,emotion_score NUMBER,timeOfvisit TIMESTAMP)')

def add_prediction_details(raw_text,pred_sentiment,sen_score,prediction,pred_proba,timeOfvisit):
	c.execute('INSERT INTO HistoryTable(rawtext,Sentiment,sentiment_score,Emotion,emotion_score,timeOfvisit) VALUES(?,?,?,?,?,?)',(raw_text,pred_sentiment,sen_score,prediction,pred_proba,timeOfvisit))
	conn.commit()
